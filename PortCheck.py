...
def checker():
    ...
    while hostlist:
        ...
        while portlist:
            ...
            socket.setdefaulttimeout(timeout)
            try:
                s.connect((str(hostlist[0]),portlist[0]))
                y=(str(hostlist[0])+':'+str(portlist[0])+' is open\n')
                file.write(str(y))
                del portlist[0]
            except (ConnectionRefusedError,TimeoutError,socket.timeout):
                del portlist[0]
        del hostlist[0]
        portlist=list(portlist2)
    file.close()
